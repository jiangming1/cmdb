...
<div class="redcolor" style="width: 100%; text-align: center;font-size: 2rem;">排行榜</div>
<div style="width: 100%;">
    <table style="width: 100%;">
         <tr>
             <td style="width: 25%"><a href="group_active.htm" class="redcolor">＜我的团</a></td>
...
<div style="width:90%; margin: 1rem auto; height: 15rem; overflow: scroll;overflow-x:hidden;overflow-y: auto;">
     <table style="width: 100%;">
		<tr>
			<td style="width: 33%; text-align: center;">排名</td>
			<td style="width: 33%; text-align: center;">团长</td>
			<td style="width: 33%; text-align: center;">团员数</td>
		</tr>
		<c:choose>
			<c:when test="${rankingList != null && fn:length(rankingList) > 0}">
				<c:forEach items="${rankingList }" var="ranking" varStatus="status">
					<tr>
						<td style="width: 33%; text-align: center;">${status.index + 1 }</td>
						<td style="width: 33%; text-align: center;">${fn:substring(ranking.headerMobile, 0, 3)}****${fn:substring(ranking.headerMobile, 7, 11)}</td>
						<td style="width: 33%; text-align: center;">${ranking.count + 1}</td>
					</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>
				<tr>
					<td style="text-align: center;" colspan="3">活动火热进行中，赶紧来报名参加吧...</td>
				</tr>
			</c:otherwise>
		</c:choose>
     </table>
...
<c:if test="${groupStatus!=null&&groupStatus!=''&&groupStatus==-1 }">
	<div class="clearfix" style="width: 100%; margin-top: 2rem;">
	    <div class="fl btn_continue" id="btn_group">我要组团抢话费</div>
	    <a href="../financial_zone.htm?tab=zone" class="fr btn_continue" id="btn_continue">年化20%新手宝</a>
	</div>
</c:if>
<!-- 活动规则内容 -->
<div class="mask"></div>
