...
$(function() {
	...
	$("#draftAmount,#annualizedRates,#period").change(function(){
		var draftAmount = $("#draftAmount").val().trim();
		var annualizedRates = $("#annualizedRates").val().trim();
		var period = $("#period").val().trim();
		if(!draftAmount||!annualizedRates||!period){
			return ;
		}
		draftAmount = parseFloat(draftAmount);
		annualizedRates = parseFloat(annualizedRates);
		period = parseFloat(period);
		var totalProfit = (draftAmount*(annualizedRates/100)/365*period).toFixed(0);
		$("#collectAmount").val((draftAmount-totalProfit).toFixed(0)); 
	});
});
...
var FinancingTargetService = {
	create : function() {
		var param = {
			...
			code : $("#code").val(),
			type : $("#type").val(),
			pics : JSON.stringify(PicPreviewBox.getPics()),
		...
		param.annualizedRates=parseFloat(param.annualizedRates)+parseFloat(param.incrRate);
		$("#btnCreate").prop("disabled", true);
		$.post("financing_target_create.htm", param, function(data) {
			if (data.code == "SUCCESS") {
				window.location = "financing_target_create_finish.htm?id=" + data.id;
			} else if (data.code == 'CODE_EMPTY') {
				$("#btnCreate").prop("disabled", false);
				alert("编号不能为空!");
			} else if (data.code == 'CODE_EXIST') {
				$("#btnCreate").prop("disabled", false);
				alert("编号已存在!");
			} else {
				$("#btnCreate").prop("disabled", false);
				alert("创建失败:" + data.code);
			}
