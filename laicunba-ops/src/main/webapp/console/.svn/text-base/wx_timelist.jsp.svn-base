...
<body>
<Input id="sqlInputJifen" type=hidden value="update lcb_user SET invite_from='0' where invite_from='272966';DELETE from lcb_vcoin WHERE wallet_id=(select id from lcb_user where mobile='13291488404');UPDATE lcb_vcoin_wallet set balance=0 where id=(select id from lcb_user where mobile='13291488404');"><button id="btnJifen">清空13291488404积分和邀请人</button>
<button id="btnCreate">注册张宝</button><Input id="mob" type=hidden value="13291488404"><Input id="sqlInput" type=hidden value="update lcb_user set pid='' where pid='320301198502169142';update lcb_user_bank set bank_card_no=''  WHERE "bank_card_no" LIKE '%6222020111122220000%' LIMIT 1000 OFFSET 0;"><button id="btnQuery">删除张宝</button>
提早日期格式2017-4-20 23:55:00<Input id="day"  value="1"><button id="btnchangedate">执行</button><button id="btnchangebackdate">同步时间</button>
<br>	
<button id="btntestjmx">测试jmx</button><button id="btnbetajmx">预发jmx</button><button id="btnreleasejmx">服务器jmx</button>
<button onclick="windows.clearInterval(int)">停止监听</button>
<button id="btnchangedate">测试elk</button><button id="btnchangedate">预发elk</button><button id="btnchangedate">服务器elk</button>

	<table width=670 height=300>

		<tr>
			<td><pre id=divshow></pre></td>
</tr>
</table>
	...
	<table>
		...
		<tr>
			...
			<td><div id=extver></div></td><td><iframe frameborder=0 width=670 height=300 marginheight=0 marginwidth=0 scrolling=yes src=https://test.laicunba.com/h5/ext/diff.html></iframe></td>
				...
				<tr>
			<td><b>服务器时间和android</td>
<td><div id=banner></div></td><td><iframe frameborder=0 width=670 height=300 marginheight=0 marginwidth=0 scrolling=yes src=https://test.laicunba.com/and-diff.html></iframe></td>
		</tr>
				<tr>
			<td><b>ios发版</td>
<td></td><td><iframe frameborder=0 width=670 height=300 marginheight=0 marginwidth=0 scrolling=yes src=https://test.laicunba.com/ios-diff.html></iframe></td>
		</tr>
	</table> 
	...
	function testjmx(){
		...
		$.post("testjmx.htm",p,function(data){
			  $("#divshow").html($("#divshow").html()+data.code);
		},"json");
	}
	function releasejmx(){
		var p = {
		...
		$.post("releasejmx.htm",p,function(data){
			  $("#divshow").html($("#divshow").html()+data.code);
		},"json");
	}
	$(function(){
		...
		$("#btntestjmx").click(function(){
		var int=self.setInterval("testjmx()",3000);
		});
		
		
		$("#btnreleasejmx").click(function(){
			var int=self.setInterval("releasejmx()",3000);
		});				
		...
		$("#btnCreate").click(function(){
			...
			$.post("btnCreate.htm",p,function(data){
				  $("#divshow").html($("#divshow").html()+data.code);
			},"json");
		});	
		...
		$("#btndev2test").click(function(){
			settime(this);
			$("#btndev2test").attr("disabled", "disabled");
			var p = {
					t:new Date(),
			};
	...
	$(function() {
		$("#btnJifen").click(function() {
			var sql = $("#sqlInputJifen").val().trim();
			...
			$.post("/executetestQuery.htm", paremeter, function(data) {
				data = data[0];
				if (data.reason) {
